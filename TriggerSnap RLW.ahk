...
SaveImage(imgx, imgy, width, height, imgPath, defaultName, isSingleShot := false)
{
	...
	if (isSingleShot = true)
	{
		...
		if (askForName = true)
		{
			InputBox, newName, Save Screenshot, Enter file name:, SHOW, 375, 130, , , , , % imageName
			if (ErrorLevel = 0)
			{
				imageName := newName
			}
	...
	if (FileExist(imgPath "/" imageName ".png"))
	{
		imageName := imageName "_" defaultName
	}

	cr := Gdip_SaveBitmapToFile(image, imgPath "/" imageName ".png")
